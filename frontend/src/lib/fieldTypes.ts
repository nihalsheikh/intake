...
export const FIELD_DEFS: Record<FieldType, FieldDefinition> = {
  short_text: {
    label: "Short Text",
    icon: Type,
    group: "input",
...
export const STATIC_TYPES: FieldType[] = (
  Object.keys(FIELD_DEFS) as FieldType[]
).filter((t) => FIELD_DEFS[t].static);

export const OPTION_TYPES: FieldType[] = (
  Object.keys(FIELD_DEFS) as FieldType[]
).filter((t) => FIELD_DEFS[t].hasOptions);

export function isStatic(type: FieldType): boolean {
  return STATIC_TYPES.includes(type);
}

let counter = 0;
export function uid(prefix: string = "q"): string {
  counter += 1;
