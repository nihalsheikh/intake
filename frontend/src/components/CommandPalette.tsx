...
export const CommandPalette = ({ open, setOpen }: CommandPaletteProps) => {
  ...
  const items: PaletteItem[] = useMemo(() => {
    ...
    const matchedActions: PaletteItem[] = actions
      .filter((a) => !q || a.label.toLowerCase().includes(q))
      .map((a) => ({ ...a, kind: "action" }));

    const matchedForms: PaletteItem[] = forms
      ...
      .map((f) => ({
        ...
        icon: FileText,
        kind: "form",
        run: () => navigate(`/builder/${f._id || f.id}`),
      }));
        ...
        <div className="max-h-80 overflow-y-auto scrollbar-thin p-2">
          ...
          {items.map((item, i) => (
            ...
            >
              ...
              {item.kind === "form" && (
                <span className="text-[10px] uppercase text-muted">Form</span>
              )}
              {active === i && (
