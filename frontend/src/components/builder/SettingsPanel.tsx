...
import { THEME_LIST, type FormThemeItem } from "@/lib/themes";
...
const ThemeThumb = ({ theme, accent }: ThemeThumbProps) => {
  return (
...
const Insight = ({ icon: Icon, label, value }: InsightProps) => {
...
export const SettingsPanel = ({ form, onChange }: SettingsPanelProps) => {
  const [summary, setSummary] = useState<FormSummaryResult | null>(null);
    ...
    <div className="space-y-6">
      ...
      <Section title="Branding">
        ...
        <div className="grid grid-cols-6 gap-2">
          {COLORS.map((c) => {
            const active =
              form.settings.primaryColor?.toLowerCase() === c.toLowerCase();
            return (
              <button
                key={c}
                type="button"
        ...
        <div className="mt-3">
          ...
          <input
            ...
            onChange={(e) =>
              setSettings({ borderRadius: Number(e.target.value) })
            }
      ...
      <Section title="Messages">
        ...
        <div className="mt-3 rounded-xl border border-default bg-surface-2 p-3">
          <Switch
            ...
            onChange={(v: boolean) => setSettings({ showProgressBar: v })}
