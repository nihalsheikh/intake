...
import { CHART, CHART_PALETTE, tooltipStyle } from "@/lib/charts";
import { shade, hexToRgba } from "@/lib/utils";
...
const HEAT_DAYS: readonly string[] = [
  "Sun",
  "Mon",
  "Tue",
  "Wed",
...
const HEAT_HOURS: readonly string[] = [
  "12a",
  "",
  "4a",
  "",
...
// Weekday × time-of-day activity heatmap
export const Heatmap = ({ data = [], color = CHART.teal }: HeatmapProps) => {
          ...
          <div key={d} className="flex items-center gap-1.5">
            ...
            {row.map((c, h) => (
              <div
                ...
                style={{
                  background: c
                    ? hexToRgba(color, 0.14 + 0.86 * (c / max))
                    : "var(--surface-2)",
                }}
              />
            ))}
...
// Semicircle gauge drawn as SVG with a gradient arc
export const Gauge = ({
  value = 0,
  suffix = "%",
  label,
...
// Donut with side legend and optional center label
export const Donut = ({ data, centerLabel, centerValue }: DonutProps) => {
...
export const RankedBars = ({ data }: RankedBarsProps) => {
