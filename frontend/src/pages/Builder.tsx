...
import { createField, FIELD_DEFS } from "@/lib/fieldTypes";
...
export type BuilderTab = "build" | "design";
...
export default function Builder() {
    ...
    async function init() {
      ...
      try {
        if (id === "new") {
          ...
          if (!active) return;
          formIdRef.current = created._id || created.id || null;
          reset(created);
          navigate(
            `/builder/${created._id || created.id}${searchParams.get("ai") ? "?ai=1" : ""}`,
            {
              replace: true,
            },
          );
          ...
          if (!active) return;
          formIdRef.current = loaded._id || loaded.id || null;
          reset(loaded);
        }
  ...
  const togglePublish = async () => {
    if (!formIdRef.current || !form) return;
    setPublishing(true);
    try {
      const isCurrentlyPublished =
        Boolean(form.published) || (form as any).status === "published";
      const updated: any = await formApi.publish(
        formIdRef.current,
      ...
      const willBePublished =
        updated?.status === "published" || Boolean(updated?.published);
      toast.success(willBePublished ? "Form published 🚀" : "Form unpublished");
      if (willBePublished) setShareOpen(true);
    } catch (err: any) {
      ...
      <header className="flex h-16 shrink-0 items-center gap-3 border-b border-default bg-surface px-3 lg:px-4">
        ...
        <div className="ml-auto flex items-center gap-1">
          ...
          <Button
            variant="ghost"
            size="icon"
            onClick={() => navigate(`/forms/${formKey}/responses`)}
          ...
          <Button
            variant="ghost"
            size="icon"
            onClick={() => navigate(`/forms/${formKey}/analytics`)}
...
function DragPreview({ drag }: DragPreviewProps) {
  ...
  const IconComponent = def.icon as ComponentType<any>;
...
function CanvasDropArea({ hasFields, children }: CanvasDropAreaProps) {
