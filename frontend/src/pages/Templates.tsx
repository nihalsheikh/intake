...
interface BuiltTemplate extends TemplateDefinition {
  questions: FormField[];
...
export default function Templates() {
  ...
  const built = useMemo<BuiltTemplate[]>(
    () =>
      TEMPLATES.map((t: any) => {
        const def = t.build();
        return {
          ...
          questionCount: def.questions.filter((q: any) => !isStatic(q.type))
            .length,
        };
      }),
    [],
  ...
  const filtered = useMemo<BuiltTemplate[]>(
    () =>
      category === "All" ? built : built.filter((t) => t.category === category),
    [built, category],
  );

  const applyTemplate = async (template: BuiltTemplate) => {
    setCreatingId(template.id);
    try {
      const form: any = await formApi.create(template.build());
      toast.success("Form created from template");
      navigate(`/builder/${form._id || form.id}`);
    } catch (err: any) {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 lg:px-8">
      ...
      <div className="mt-7 flex flex-wrap gap-1.5">
        {TEMPLATE_CATEGORIES.map((cat: string) => (
          <button
            key={cat}
            type="button"
